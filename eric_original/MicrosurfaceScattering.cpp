...
inline double eric_gamma (double x)
{
  double  result;
...
inline double  beta (double m, double n)
{
	...
	else // transmission
	{
		...
		if(wi_outside){
			value = eta*eta * (1.0f-Fresnel(wi, wh, eta)) *
				m_microsurfaceslope->D_wi(wi, wh) * fmaxf(0.0f, -dot(wo, wh)) *
